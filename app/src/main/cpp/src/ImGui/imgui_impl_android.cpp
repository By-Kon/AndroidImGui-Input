...
int32_t ImGui_ImplAndroid_HandleInputEvent(AInputEvent* input_event)
{
    ...
    switch (event_type)
    {
        case AINPUT_EVENT_TYPE_KEY:
        {
            ...
            switch (event_action)
            {
                 // FIXME：一旦触摸指针，AKEY_EVENT_ACTION_DOWN 和 AKEY_EVENT_ACTION_UP 就会立即发生
                // 从一个键上升。 我们使用一个简单的按键事件队列/并在每帧中处理每个按键一个事件
                // ImGui_ImplAndroid_NewFrame()...或者考虑使用 IO 队列，如果合适的话：https://github.com/ocornut/imgui/issues/2787
        ...
        case AINPUT_EVENT_TYPE_MOTION:
        {
            ...
            switch (event_action)
            {
                ...
                case AMOTION_EVENT_ACTION_UP:
                    // 物理鼠标按钮（可能还有其他物理设备）也会调用操作 AMOTION_EVENT_ACTION_DOWN/_UP，
                    // 但我们必须单独处理它们以识别实际按下的按钮。 这是通过下面完成的
                    // AMOTION_EVENT_ACTION_BUTTON_PRESS/_RELEASE。 在这里，我们只处理“FINGER”输入（以及“UNKNOWN”，作为后备）。
